...
/**
 * logging function for aikgen2
 */
static void aikgen2_dbg(debug_t group, level_t level, char *fmt, ...)
{
	char buffer[8192];
 ...
 */
static void exit_aikgen2(err_t message, ...)
{
	int status = 0;
...
{
	fprintf(stderr,
		"Usage: aikgen2  [--aikblob <filename>] [--aikpubkey <filename>]"
		" [--aikname <filename>] [--force]\n"
		"                [--quiet] [--debug <level>]\n"
		"       aikgen2 --help\n"
		"\n"
		"Options:\n"
		" --aikblob (-b)    TSS 2.0 blob with AIK public key\n"
	...
	exit_aikgen2(message);
}

/**
 * @brief main of aikgen2 which generates an Attestation Identity Key (AIK)
 *
...
int main(int argc, char *argv[])
{
	...
	if (!library_init(NULL, "aikgen2"))
	{
		exit(SS_RC_LIBSTRONGSWAN_INTEGRITY);
	}
	if (lib->integrity &&
		!lib->integrity->check_file(lib->integrity, "aikgen2", argv[0]))
	{
		fprintf(stderr, "integrity check of aikgen2 failed\n");
	...
	for (;;)
	{
		static const struct option long_opts[] = {
			...
			{ "aikname", required_argument, NULL, 'n' },
		...
		switch (c)
		{
			...
			case '+':       /* --optionsfrom <filename> */
				...
				{
					exit_aikgen2("optionsfrom failed");
				}
				continue;
			...
			case 'n':       /* --aikname <filename> */
				aikname_filename = optarg;
				continue;
	...
	if (!lib->plugins->load(lib->plugins,
			lib->settings->get_str(lib->settings, "aikgen2.load", PLUGINS)))
	{
		exit_aikgen2("plugin loading failed");
	}
	...
	if (!aikblob)
	{
		exit_aikgen2("could not read TSS 2.0 public key file '%s'",
					  aikblob_filename);
	}
	...
	{
		chunk_unmap(aikblob);
		exit_aikgen2("size of aikblob is not %d bytes", sizeof(TPM2B_PUBLIC));
	}
	...
	switch (public.t.publicArea.type)
	{
		case TPM_ALG_RSA:
		{
			...
			{
				exit_aikgen2("subjectPublicKeyInfo encoding of AIK key failed");
			}
			break;
		}
		case TPM_ALG_ECC:
	...
	if (!hasher)
	{
		exit_aikgen2("SHA1 hash algorithm not supported");
	}
	if (!hasher->allocate_hash(hasher, aik_pubkey, &aik_keyid))
	{
		hasher->destroy(hasher);
		exit_aikgen2("computing SHA1 fingerprint failed");
	}
	hasher->destroy(hasher);
